...
// ─── MCP Server factory ───────────────────────────────────────────────────────
// A new instance is created per SSE connection to avoid "already connected" errors
function createServer() {
  ...
  server.tool("get_athlete_profile",
    ...
    async () => {
      try {
        ...
        if (lines.length <= 2) {
          return { content: [{ type: "text", text: `Raw: ${JSON.stringify(data, null, 2)}` }] };
        }
  ...
  server.tool("get_activity_detail",
    "Deep detail for one activity: splits, HR zones, pace zones. Use ID from get_activities.",
  ...
  server.tool("get_weekly_stats",
    "Weekly training totals: km, duration, sessions, TSS, calories. Great for load trend analysis.",
    ...
    async ({ weeks = 8 }) => {
      try {
        ...
        const lines  = [`📊 WEEKLY STATS (${w} weeks)\n${"─".repeat(40)}`];
...
// SSE endpoint — no express.json() here, let the SDK handle the response
...
// Messages endpoint — needs JSON parsing
